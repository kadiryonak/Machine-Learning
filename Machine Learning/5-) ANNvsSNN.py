...
class DataAnalyzer:
    def __init__(self, X, y):
        self.X = X
        self.y = y
        self.df = pd.DataFrame(X, columns=["Feature_" + str(i) for i in range(1, X.shape[1] + 1)])
    ...
    def train(self):
        for epoch in range(1, self.epochs + 1):
            ...
            for i in range(0, self.X_train.size(0), self.batch_size):
                ...
                loss.backward()
                self.optimizer.step()
            if epoch % 10 == 1 or epoch == self.epochs:
                print(f"Epoch [{epoch}/{self.epochs}], Loss: {loss.item():.4f}")
        print("ANN Eğitimi Tamamlandı!")
...
class Main:
    ...
    def run(self):
        ...
        print("İstatistiksel Özet:", data_analyzer.describe_data())
        print("\nSınıf Dağılımı:", data_analyzer.class_distribution())
        print("\nÖzellikler Arası Korelasyon:", data_analyzer.feature_correlation())
